...
DBR = 25 * [
    Layer(d_MgF2*1e-9, MgF2),
    Layer(d_TiO2*1e-9, TiO2)
]

wavelengths = np.linspace(300, 1850, 300) * 1e-9

photon_flux = LightSource(source_type='standard', version='AM0', x=wavelengths,
                          output_units='photon_flux_per_m').spectrum(wavelengths)[1]

grating_circles = [
    {"type": "circle", "mat": InGaP, "center": (0, 0), "radius": 185}
]
...
S4_setup = rcwa_structure(
    layers, size=size, options=options, incidence=Air, transmission=Ge
)
...
plt.plot(wavelengths*1e9, RAT['A_per_layer'][:,3], label='InGaP absorption',
         color=pal[0])
plt.plot(wavelengths*1e9, RAT['A_per_layer'][:,4], label='GaAs absorption',
         color=pal[1])
plt.plot(wavelengths*1e9, RAT['T'], label='Transmission into Ge',
         color=pal[2])
plt.legend(loc=(1.05, 0.6))
plt.xlim(300, 1850)
plt.ylim(0, 1)
plt.text(500, 0.5, 'InGaP: \n' + str(round(J_InGaP, 1)) + r' mA/cm$^2$',
         horizontalalignment='center')
plt.text(760, 0.4, 'GaAs: \n' + str(round(J_GaAs, 1)) + r' mA/cm$^2$',
         horizontalalignment='center')
plt.text(1500, 0.85, 'Ge: \n' + str(round(J_Ge, 1)) + r' mA/cm$^2$',
            horizontalalignment='center')
...
tmm_setup = tmm_structure(
    layers, incidence=Air, transmission=Ge
)
...
plt.plot(wavelengths*1e9, RAT_nograting['A_per_layer'][:,4],
         label='700 nm GaAs + DBR', color=pal[0])
plt.plot(wavelengths*1e9, RAT_noDBR['A_per_layer'][:,4], label='700 nm GaAs + grating',
         color=pal[1])
plt.plot(wavelengths*1e9, RAT['A_per_layer'][:,4], label='700 nm GaAs + grating + DBR',
         color=pal[2])
...
labels = ['700 nm GaAs', '700 nm GaAs + grating',
            '700 nm GaAs + DBR', '700 nm GaAs + grating + DBR',
          '3500 nm GaAs']

A_list = [RAT_700['A_per_layer'][:,4],
            RAT_nograting['A_per_layer'][:,4], RAT_noDBR['A_per_layer'][:,4],
          RAT['A_per_layer'][:,4],
          RAT_ref1['A_per_layer'][:,4]]

T_list = [RAT_700['T'],
            RAT_nograting['T'], RAT_noDBR['T'],
          RAT['T'],
          RAT_ref1['T']]
