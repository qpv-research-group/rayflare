...
wavelengths = np.linspace(300, 1000, 250)*1e-9
...
test_mat = [100, wavelengths*1e9, GaAs_n_tensor, GaAs_k_tensor, []]
test_mat2 = [1000, wavelengths*1e9, Si_n_tensor, Si_k_tensor, []]

rcwa_setup = rcwa_structure([Layer(100e-9, GaAs), Layer(1e-6, Si)], size=((100, 0), (0, 100)), options=options, incidence=Air, transmission=Ag)

rcwa_setup_AS = rcwa_structure([test_mat, test_mat2], size=((100, 0), (0, 100)), options=options, incidence=Air, transmission=Ag)
...
plt.plot(prof_AS['profile'][[0,20,50,100,249],:].T, '--')
plt.show()
