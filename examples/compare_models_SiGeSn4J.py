...
plt.plot(options['wavelengths']*1e9, results_per_layer_front[:,0], label='MgF2')
plt.plot(options['wavelengths']*1e9, results_per_layer_front[:,1], label='Ta2O5')
...
plt.plot(options['wavelengths']*1e9, results_per_layer_back[:,0], label='SiN')
...
front_surf = Interface('RT_TMM', layers=front_materials, texture=surf, name = 'GaInP_GaAs_SiGeSn_RT',
                       coherent=True)
back_surf = Interface('RT_TMM', layers=back_materials, texture = surf, name = 'SiN_Ag_RT',
                      coherent=True)
...
TMM_res = calculate_rat(all_layers, wavelength=wavelengths*1e9, substrate=Ag, no_back_reflection=False, angle=options['theta_in']*180/np.pi, coherent=False,
...
plt.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][len(front_materials)+1], label='A_bulk (Ge)')
plt.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][1], label='MgF2')
plt.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][2], label='Ta2O5')
