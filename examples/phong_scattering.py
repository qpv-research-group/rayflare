...
triangle_phong_surf = regular_pyramids(52, upright=True, phong=True,
                                       phong_options=[10000, False])

phong_surf = planar_surface(phong=True, phong_options=[10000, False])  # pyramid size in microns
...
ax1.hist(np.pi-rat_scat["thetas"][0], color=pal[2], alpha=0.5, label="Phong 100 % transmission",
...
n, bins, patches = plt.hist((np.cos(np.pi-rat_scat["thetas"][0])), color=pal[2], alpha=0.5,
...
for bin_i in range(n_bins-1):
    ...
    apparent_brightness[bin_i] = d_omega*n_rays_in_bin/area_strip
...
# for each ray, find it's intensity
