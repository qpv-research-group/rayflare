...
plt.hist(h_dist, bins=15, range=[mean_h-2*std_h, mean_h+2*std_h],
         ...
         histtype='step',
         color='k',
         label="Pyramids", linestyle='--')
plt.hist(h_dist_alk, bins=20, range=[0, 0.7],
         density=True,
         histtype='step',
         color='k',
         label="Alkaline", linestyle='--')
plt.show()
...
names = ["sample3_pyramid.Sample.Raw", "sample3_pyramid.Sample.Raw"]
# names = ["2_pyramid.Sample.Raw"]
...
for i1, name in enumerate(names):
    ...
    RT = np.loadtxt(path_RT + "/" + name + '.csv', skiprows=1, delimiter=",")
    R = np.loadtxt(path_R + "/" + name + '.csv', skiprows=1, delimiter=",")
    T = np.loadtxt(path_T + "/" + name + '.csv', skiprows=1, delimiter=",")
    # plt.plot(RT[:,0], RT[:,1], '-k')
    plt.plot(R[:, 0], R[:, 1]/100, '--')
    plt.plot(T[:, 0], T[:, 1]/100, '-.')
    plt.plot(T[:, 0], (100 - (R[:, 1] + T[:, 1]))/100, '--r')
#
# plt.show()
