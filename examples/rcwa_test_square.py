...
all_orders = [3, 9, 19, 39, 75, 99, 125, 200, 300]
all_orders = [400]
...
if calc:
    ...
    for i1, orders in enumerate(all_orders):
        ...
        to_save = np.vstack((wavelengths, output['A_layer'].todense()[:, 1, :][:, 0],
                             ...
                             output['R'][:,0],
                             output['T'][:,0])).T
        np.savetxt('A_GaAs_sq_' + str(orders) +'.csv', to_save, delimiter=',')
    ...
    for i1, orders in enumerate(load_orders):
        A_GaAs = np.loadtxt('A_GaAs_sq_' + str(orders) + '.csv', delimiter=',')
...
plt.plot(EQE_wl, solar_cell[1].layer_absorption, label='TMM')
plt.show()
