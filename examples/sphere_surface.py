...
if os.path.isfile('sphere_raytrace.txt'):

    T_values = np.loadtxt('sphere_raytrace.txt')
    T_total = np.loadtxt('sphere_raytrace_totalT.txt')
    ...
    np.savetxt('sphere_raytrace.txt', T_values)
    np.savetxt('sphere_raytrace_totalT.txt', T_total)


plt.figure()
plt.plot(thetas, T_values, 'o-', label='Escape within max. angle')
plt.plot(thetas, T_total, 'o-', label='Total escape from front')
plt.legend()
plt.xlim(0, np.pi/2)
