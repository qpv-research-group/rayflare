...
for i1, nx in enumerate(nxs):
    ...
    if os.path.isfile('sphere_raytrace_2e12_' + str(options.n_rays) + 'rays.txt'):

        T_values = np.loadtxt('sphere_raytrace_2e12_' + str(options.n_rays) + 'rays.txt')
        T_total = np.loadtxt('sphere_raytrace_totalT_2e12_' + str(options.n_rays) + 'rays.txt')
        ...
        np.savetxt('sphere_raytrace_2e12_' + str(options.n_rays) + 'rays.txt', T_values)
        np.savetxt('sphere_raytrace_totalT_2e12_' + str(options.n_rays) + 'rays.txt', T_total)
...
#     plt.axvline(x=ln)
plt.title(r'Convergence - number of rays ($N_{triangles} = 2^{12}$)')
