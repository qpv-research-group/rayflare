...
thetas = np.linspace(0.78, 0.96, 6)
...
for i1, nx in enumerate(nxs):
    ...
    for k1, pol in enumerate(['s', 'p']):
        ...
        if os.path.isfile('sphere_raytrace_2e13_' + str(options.n_rays) + 'rays' + pol + 'coarse.txt'):

            T_values = np.loadtxt('sphere_raytrace_2e13_' + str(options.n_rays) + 'rays' + pol + 'coarse.txt')
            T_total = np.loadtxt('sphere_raytrace_totalT_2e13_' + str(options.n_rays) + 'rays' + pol + 'coarse.txt')
            ...
            np.savetxt('sphere_raytrace_2e13_' + str(options.n_rays) + 'rays' + pol + 'coarse.txt', T_values)
            np.savetxt('sphere_raytrace_totalT_2e13_' + str(options.n_rays) + 'rays' + pol + 'coarse.txt', T_total)



        # plt.plot(thetas*180/np.pi, T_values, 'o-', label=pol, color=pal[k1])
        # plt.plot(thetas*180/np.pi, T_total, 'o--', color=pal[k1])
        # plt.legend()
        plt.xlim(0, 90)
        ...
        n_interactions_list.append(n_interactions)
...
T_unpol = np.mean(T_list, 0)
T_unpol_total = np.mean(T_total_list, 0)
#
T_values_u = np.loadtxt('sphere_raytrace_2e13_' + str(1156) + 'rays.txt')
T_total_u = np.loadtxt('sphere_raytrace_totalT_2e13_' + str(1156) + 'rays.txt')

thetas_all = np.linspace(0, np.pi / 2 - 0.02, 75)

plt.figure()
plt.plot(thetas * 180 / np.pi, T_unpol, 'ko-', label='s/p average')
plt.plot(thetas * 180 / np.pi, T_unpol_total, 'ko--')
plt.plot(thetas_all * 180 / np.pi, T_values_u, 'ro-', label='u-pol rays')
plt.plot(thetas_all * 180 / np.pi, T_total_u, 'ro--')
plt.legend()
plt.show()

from rayflare.ray_tracing.rt import calc_R

thetas_r = np.linspace(0, np.pi/2, 1000)

R_s = calc_R(3.324, 1, thetas_r, 's')
R_p = calc_R(3.324, 1, thetas_r, 'p')



plt.figure()
plt.plot(thetas_r*180/np.pi, R_s, label='s')
plt.plot(thetas_r*180/np.pi, R_p, label='p')
plt.legend()
plt.show()
