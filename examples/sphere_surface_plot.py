...
X, Y, Z = points_on_sphere(2**14, radius=0.8, h=0.242)
...
thetas = np.linspace(0, np.pi-0.1, 30)

T_values = np.zeros(len(thetas))


result = rtstr.calculate(options)
T_values= np.sum(result['thetas'] > minimum_angle)/options.n_rays
print('Transmission', result['T'])

plt.show()

plt.figure()
plt.hist(result['thetas'][result['thetas']>np.pi/2])
plt.show()