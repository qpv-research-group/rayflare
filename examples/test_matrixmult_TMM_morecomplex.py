...
SC = Structure([front_surf, bulk_Si, back_surf], incidence=Air, transmission=SiN)
...
plt.plot(options['wavelengths']*1e9, results_per_layer_back[:,0], label='InGaP back')
plt.plot(options['wavelengths']*1e9, results_per_layer_back[:,1], label='GaAs back')
...
layers = [Layer(60e-9, SiN), Layer(50E-9, GaInP), Layer(50e-9, GaAs), Layer(20e-6, Si), Layer(100E-9, GaInP), Layer(70e-9, GaAs)]

from solcore.absorption_calculator import OptiStack, calculate_rat

optist = OptiStack(layers, incidence=Air, substrate=SiN, no_back_reflection=False)
TMM_res = calculate_rat(optist, options['wavelengths']*1e9, angle=options['theta_in']*180/np.pi, pol=options['pol'],
                        coherent=False, coherency_list=['c', 'c', 'c', 'i', 'c', 'c'], no_back_reflection=False)
...
plt.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][5], label='InGaP back')
plt.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][6], label='GaAs back')
plt.legend()
plt.show()