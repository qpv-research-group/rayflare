...
options.project_name = 'back_incidence'
options.n_rays = 250
...
front_materials = [Layer(100e-9, MgF2), Layer(100e-9, Ta2O5), Layer(200e-9, GaAs)]
...
ax1.plot(options['wavelengths']*1e9, results_per_layer_front[:,2], label='GaAs')
...
ax2.plot(options['wavelengths']*1e9, results_per_layer_front[:,2], label='GaAs')
...
TMM_res = OS_layers.calculate(options, profile=True, layers=[1,3,4,5])
...
ax4.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][:,2], label='GaAs')
ax4.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][:,3], label='SiN')
ax4.plot(options['wavelengths']*1e9, TMM_res['A_per_layer'][:,4], label='back')
