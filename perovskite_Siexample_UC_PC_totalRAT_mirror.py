...
Jph_Si_text = q*np.trapz(photon_flux*RAT['A_bulk'], options['wavelengths'])[0]*1000/(100**2)  # A/m^2
...
photon_flux = light_source._get_photon_flux_per_m(options['wavelengths'])
Jph_Si_flat = q*np.trapz(photon_flux*RAT['A_bulk'], options['wavelengths'])[0]*1000/(100**2)  # A/m^2
Jph_Perovskite = q*np.trapz(photon_flux*results_per_layer_front[:,3], options['wavelengths'])*1000/(100**2)
#
# Plot
import seaborn as sns
...
fig = plt.figure()
ax = plt.subplot(111)
ax.plot(options['wavelengths']*1e9, bulk_A_text, color= pal[0], linewidth=2,
        label='Textured back surface')
ax.plot(options['wavelengths']*1e9, bulk_A_flat, color= pal[5], linewidth=2,
...
fig.savefig('textcomp', bbox_inches='tight')
plt.show()
