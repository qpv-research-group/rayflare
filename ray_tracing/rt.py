...
# the return from scipy.spatial.Delaunay, but contains 3D rather than 2D coordinates,
...
def RT(group, incidence, transmission, options):
    ...
    for i1 in range(len(group.materials)):
        ...
        n_depths.append(group.n_depths[i1])
    ...
    for i1, wl in enumerate(options['wavelengths']):
        materials = {'nk': nks[:,i1], 'alpha': np.imag(nks[:,i1])*4*np.pi/(wl*1e6), 'width': widths}

        I, profiles, th_o, ray_path = single_ray(0.1, 0.25, theta, phi, surfaces, materials, n_depths, I_thresh)
        absorption_profiles.append(profiles)
        #
        if th_o is not None:
    ...
    return R, T, absorption_profiles
...
def single_ray(x, y, theta, phi, surfaces, materials, n_depths, I_thresh):
    ...
    # for now, just do everything in microns

    # need to calculate r_a and r_b
    profiles = []
    depths = []
    for i1, n in enumerate(n_depths):  # don't care about absorption profile in incidence and transmission medium
        profiles.append(np.zeros(n))
    ...
    h = max(surfaces[surf_index].Points[:, 2])
    r = abs((h + 1) / cos(theta))
    ...
    while not stop:
        ...
        DA, stop, I, theta = traverse(materials['width'][mat_index], theta, materials['alpha'][mat_index], x, y, I,
                                      depths[mat_index], I_thresh, direction)
        n_passes = n_passes + 1
        #print(DA)
        profiles[mat_index] = profiles[mat_index] + DA
    ...
    return I, profiles, theta, ray_path
