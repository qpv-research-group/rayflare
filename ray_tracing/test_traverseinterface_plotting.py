...
from traverse_interface_plotting_noTIR import single_ray, RTSurface, calc_R
...
z2 = np.array([0, h, 0, 0, 0])

Points = np.vstack([x2, y2, z-5]).T
...
ax.plot_trisurf(x3, y3, z3-11, triangles=surf3.simplices,  linewidth=1, color = (0.5, 0.5, 0.5, 0.5))
ax.set_xlabel('X')
ax.set_ylabel('Y')
#ax.set_zlim(-9, -10.5)
#ax.set_xlim(0, 1.5)

surfaces = [surf, surf2, surf3]
materials = {'nk': [1, 4+0.5*1*1j, 2.5+0.5*1j, 1], 'alpha': [0, 0.1, 0.1, 0], 'width': [0, 10, 5, 0]}
#materials = {'nk': [1, 3+0.5*1*1j], 'alpha': [0, 0.05], 'width': [0, 0]}

I_thresh = 1e-6

n_points = [0, 100, 100, 0]
...
I, profile, theta, ray_path = single_ray(x, y, theta, phi, surfaces, materials, n_points, I_thresh, ax)
