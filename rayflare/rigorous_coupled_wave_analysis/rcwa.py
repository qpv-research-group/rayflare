...
def RCWA(
    ...
    if existing_mats:
        ...
        for i1, x in enumerate(layers):
            layers_oc[:, i1 + 1] = (
                x.material.n(wavelengths) + 1j * x.material.k(wavelengths)
            ) ** 2
...
def RCWA_wl(
    ...
    if side == 1:
        ...
        binned_theta_in = (
            np.digitize(np.pi - np.pi * theta / 180, theta_intv, right=True) - 1
        )
        phi_in = xr.DataArray(
            np.pi * phi / 180,
    ...
    for i1, (th, ph) in enumerate(zip(theta, phi)):
        if pol in "sp":
            ...
            S.SetFrequency(1 / wl)
            out, R_pfbo, T_pfbo, R_pfbo_int = rcwa_rat(
                S, len(widths), th, n_inc, layer_details
            )
            T[in_bin[i1]] = out["T"]
            ...
            R_pfbo = (
                R_pfbo_s + R_pfbo_p
            )  # this will not be normalized correctly! fix this outside if/else
        ...
        if dist is not None:
            ...
            if len(pol) == 2:
                ...
                for j, d in enumerate(dist):
                    ...
                    data = rcwa_position_resolved(
                        S, layer_name, d_in_layer, A, th, np.sqrt(l_oc[0])
                    )
                ...
                if pol in "sp":
                    ...
                    for j, d in enumerate(dist):
                        ...
                        S.SetExcitationPlanewave((th, ph), 0, 1, 0)  # p-polarization
                        data_p = rcwa_position_resolved(
                            S, layer_name, d_in_layer, np.sum(Ap), th, np.sqrt(l_oc[0])
                        )
                        S.SetExcitationPlanewave((th, ph), 1, 0, 0)  # p-polarization
                        data_s = rcwa_position_resolved(
                            S, layer_name, d_in_layer, np.sum(As), th, np.sqrt(l_oc[0])
                        )
        ...
        theta_t = np.pi - np.real(
            np.arccos(ft_z / np.sqrt(fr_x**2 + fr_y**2 + ft_z**2))
        )
...
def rcwa_rat(S, n_layers, theta, n_inc, det_l=False):
    ...
    R = (
        -n_inc * np.real(S.GetPowerFlux("layer_1")[1]) / np.cos(theta * np.pi / 180)
    ...
    T_pfbo = (
        n_inc
        * np.real(np.sum(np.array(S.GetPowerFluxByOrder(below)), 1))
...
def initialise_S(
    size, orders, geom_list, mats_oc, shapes_oc, shape_mats, widths, options
):
...
def rcwa_position_resolved(S, layer, depth, A, theta, n_inc):
    if A > 0:
        ...
        return (
            n_inc * power_difference / (2 * delta * np.cos(theta * np.pi / 180))
...
def rcwa_absorption_per_layer_order(S, n_layers, theta, n_inc):
    ...
    for i1, layer in enumerate(np.arange(n_layers - 2) + 2):
        ...
        per_order_top = np.sum(
            np.array(S.GetPowerFluxByOrder("layer_" + str(layer))), 1
        )
        per_order_bottom = np.sum(
            np.array(S.GetPowerFluxByOrder("layer_" + str(layer + 1))), 1
        )

        A_per_order[i1, :] = (
            n_inc
            * np.real(per_order_top - per_order_bottom)
            / np.cos(theta * np.pi / 180)
        )
...
class rcwa_structure:
    ...
    def update_oc(self, wavelengths):
        ...
        for i1, x in enumerate(self.shape_mats):
            if isinstance(x, list):
                if len(x) == 3:
                    shapes_oc[:, i1] = (
                        np.ones_like(wavelengths) * (x[1] + 1j * x[2]) ** 2
                    )
    ...
    def get_fields(
        ...
        def vs_pol(s, p):
            S.SetExcitationPlanewave(
                (options["theta_in"] * 180 / np.pi, options["phi_in"] * 180 / np.pi),
                s,
                p,
                0,
            )
    ...
    def get_fields_unit_cell(
        self, layer_index, wavelength, options, depth=1e-10, n_points=200
    ):
        """
        Get the components of the E and H fields at a specific depth in a layer, over a range of x/y points. Can also plot results
        automatically. Uses the S4 function GetFields().
        ...
        E, H = S.GetFieldsOnGrid(
            z=depth, NumSamples=(n_points, n_points), Format="Array"
        )
...
def RCWA_structure_wl(
    ...
    def vs_pol(s, p):
        ...
        R = out[0]["R"]
        T = out[0]["T"]
        A_layer = rcwa_absorption_per_layer(
    ...
    S = initialise_S(
        size, orders, geom_list, layers_oc, shapes_oc, s_names, widths, S4_options
    )
...
def RCWA_wl_prof(
    ...
    if len(pol) == 2:
        ...
        for j, d in enumerate(dist):
            ...
            data = rcwa_position_resolved(
                S, layer_name, d_in_layer, A, theta, np.sqrt(layers_oc[0])
            )
            profile_data[j] = data
