...
def test_tmm_rcwa_structure_comparison():
    ...
    for pol in ["s", "p", "u"]:
        for angle in [0, np.pi / 3]:
            ...
            assert np.sum(tmm_result["A_per_layer"], 1) + tmm_result["R"] + tmm_result[
                "T"
            ] == approx(1)
            assert np.sum(rcwa_result["A_per_layer"], 1) + rcwa_result[
                "R"
            ] + rcwa_result["T"] == approx(1)
...
def test_planar_structure():
    ...
    front_surf = Interface(
        "TMM", layers=front_materials, name="GaInP_GaAs_TMM", coherent=True
    )
    back_surf = Interface(
        "TMM", layers=back_materials, name="SiN_Ag_TMM", coherent=True
    )
    ...
    TMM_matrix = np.hstack(
        (
            results_per_layer_front_TMM_matrix,
            results_TMM_Matrix[0].A_bulk[0].data[:, None],
        )
    )
    RCWA_matrix = np.hstack(
        (results_per_layer_front_RCWA, results_RCWA_Matrix[0].A_bulk[0].data[:, None])
    )
    RT_matrix = np.hstack(
        (results_per_layer_front_RT, results_RT[0].A_bulk[0].data[:, None])
    )
    ...
    assert (
        results_RT[0].R[0]
        + results_RT[0].T[0]
        + np.sum(results_per_layer_front_RT, 1)
        + results_RT[0].A_bulk[0]
    ).data == approx(1)
    ...
    for angle in angles:
        for pol in pols:
            ...
            front_surf = Interface(
                "TMM",
                layers=front_materials,
                name="GaInP_GaAs_TMM" + str(pol),
                coherent=True,
            )
            back_surf = Interface(
                "TMM",
                layers=back_materials,
                name="SiN_Ag_TMM" + str(pol),
                coherent=True,
            )
            ...
            SC = Structure(
                [front_surf, bulk_Ge, back_surf], incidence=Air, transmission=Ag
            )
            ...
            results_per_layer_front_TMM_matrix = np.sum(
                results_per_pass_TMM_matrix["a"][0], 0
            )
            ...
            RCWA_matrix = np.hstack(
                (
                    results_per_layer_front_RCWA,
                    results_RCWA_Matrix[0].A_bulk[0].data[:, None],
                )
            )
...
def test_absorption_profile():
    ...
    strt = tmm_structure(
        stack, incidence=Air, transmission=Air, no_back_reflection=False
    )
...
def test_rcwa_tmm_matrix_check_sums():
    ...
    for pol in ["s", "p", "u"]:
        ...
        back_surf = Interface(
            "TMM", layers=back_materials, name="TMM_b", coherent=True, prof_layers=[1]
        )
        ...
        assert np.all(
            results_TMM_Matrix[0]["A_interface"][0].data
            == np.sum(results_per_pass["a"][0], (0, 2))
        )

        assert np.all(
            results_TMM_Matrix[0]["A_interface"][1].data
            == np.sum(results_per_pass["a"][1], (0, 2))
        )
        ...
        assert np.all(
            results_RT[0]["A_interface"][0].data
            == np.sum(results_per_pass["a"][0], (0, 2))
        )

        assert np.all(
            results_RT[0]["A_interface"][1].data
            == np.sum(results_per_pass["a"][1], (0, 2))
        )
        ...
        assert np.all(
            results_RCWA_Matrix[0]["A_interface"][0]
            == np.sum(results_per_pass["a"][0], (0, 2))
        )

        assert np.all(
            results_RCWA_Matrix[0]["A_interface"][1]
            == np.sum(results_per_pass["a"][1], (0, 2))
        )
        ...
        results_per_layer_front_TMM_ref = TMM_res["A_per_layer"][
            :, : len(front_materials)
        ]
        ...
        assert results_per_layer_front_TMM[c_i] == approx(
            results_per_layer_front_TMM_ref[c_i], rel=0.05
        )
        assert results_per_layer_front_RCWA[c_i] == approx(
            results_per_layer_front_TMM_ref[c_i], rel=0.05
        )
        assert results_per_layer_front_RT[c_i] == approx(
            results_per_layer_front_TMM_ref[c_i], rel=0.8
        )

        c_i = results_per_layer_back_TMM_ref > 1e-2

        assert results_per_layer_back_TMM[c_i] == approx(
            results_per_layer_back_TMM_ref[c_i], rel=0.05
        )
        assert results_per_layer_back_RCWA[c_i] == approx(
            results_per_layer_back_TMM_ref[c_i], rel=0.05
        )
...
def test_rcwa_tmm_matrix_profiles():
    ...
    for pol in ["s", "p", "u"]:
        ...
        depths = np.linspace(
            0,
            len(prof_plot_TMM[0, :]) * options["depth_spacing"] * 1e9,
            len(prof_plot_TMM[0, :]),
        )
        ...
        depths_back = np.linspace(
            0,
            len(prof_plot_TMM_back[0, :]) * options["depth_spacing"] * 1e9,
            len(prof_plot_TMM_back[0, :]),
        )
        integrated_A_back = np.trapz(prof_plot_TMM_back, depths_back, axis=1)

        assert integrated_A == approx(
            results_TMM_Matrix[0]["A_interface"][0].data, abs=0.01
        )
        assert integrated_A_back == approx(
            results_TMM_Matrix[0]["A_interface"][1].data, abs=0.01
        )
        ...
        depths = np.linspace(
            0,
            len(prof_plot_RT[0, :]) * options["depth_spacing"] * 1e9,
            len(prof_plot_RT[0, :]),
        )
        ...
        depths_back = np.linspace(
            0,
            len(prof_plot_RT_back[0, :]) * options["depth_spacing"] * 1e9,
        ...
        assert integrated_A_back == approx(
            results_RT[0]["A_interface"][1].data, abs=0.01
        )
        ...
        depths = np.linspace(
            0,
            len(prof_plot_RCWA[0, :]) * options["depth_spacing"] * 1e9,
            len(prof_plot_RCWA[0, :]),
        )
        integrated_A = np.trapz(prof_plot_RCWA, depths, axis=1)

        depths_back = np.linspace(
            0,
            len(prof_plot_RCWA_back[0, :]) * options["depth_spacing"] * 1e9,
            len(prof_plot_RCWA_back[0, :]),
        )
        integrated_A_back = np.trapz(prof_plot_RCWA_back, depths_back, axis=1)

        assert integrated_A == approx(
            results_RCWA_Matrix[0]["A_interface"][0].data, abs=0.01
        )
        assert integrated_A_back == approx(
            results_RCWA_Matrix[0]["A_interface"][1].data, abs=0.01
        )
...
def test_compare_RT_TMM_Fresnel():
    ...
    front_surf = Interface(
        "RT_TMM",
        name="RT_TMM_f",
        texture=triangle_surf,
        layers=[Layer(1e-9, Si)],
        coherent=False,
        coherency_list=["i"],
    )
    back_surf = Interface(
    ...
    assert res_fresnel[0]["A_bulk"][0].data == approx(
        res_TMM[0]["A_bulk"][0].data, abs=0.15
    )
    assert res_fresnel[0]["R"][0].data == approx(res_TMM[0]["R"][0].data, abs=0.15)
    assert res_fresnel[0]["T"][0].data[T_nz] == approx(
        res_TMM[0]["T"][0].data[T_nz], abs=0.15
    )
...
def compare_rt_integrated_tmm():
    ...
    rtstr = rt_structure(
        [front_surf, back_surf], [Si], [d_Si], Air, Air, opts, use_TMM=True
    )
    ...
    for angle_pol in itertools.product(thetas_in, pol_in):
        ...
        # Si absorption:
        assert rt_res["A_per_layer"][:, 0] == approx(
            tmm_res["A_per_layer"][:, 2], abs=0.05
        )
        # MgF2 + GaAs absorption:
        assert rt_res["A_per_interface"][0] == approx(
            tmm_res["A_per_layer"][:, :2], abs=0.05
        )
        # Ag absorption:
        assert rt_res["A_per_interface"][1] == approx(
            tmm_res["A_per_layer"][:, -1:], abs=0.05
        )
...
def compare_rt_integrated_tmm_profile():
    ...
    front_surf = planar_surface(
        interface_layers=front_layers, prof_layers=[2]
    )  # pyramid size in microns
    back_surf = planar_surface(
        interface_layers=back_layers, prof_layers=[1]
    )  # pyramid size in microns
    ...
    for angle_pol in itertools.product(thetas_in, pol_in):
        ...
        ratio = (
            rt_front[rt_front > 3e-5]
            / tmm_res["profile"][:, : rt_front.shape[1]][rt_front > 3e-5]
        )

        assert ratio == approx(np.ones_like(ratio), abs=0.07)

        ratio = (
            rt_back[rt_back > 3e-5]
            / tmm_res["profile"][:, -rt_back.shape[1] :][rt_back > 3e-5]
        )
...
def test_tmm_arm():
    ...
    options.coherency_list = (
        ["c"] * len(front_materials) + ["i"] + ["c"] * len(back_materials)
    )
    options.coherent = False

    tmmstrt = tmm_structure(
        front_materials + [Layer(bulkthick, Ge)] + back_materials,
        incidence=Air,
        transmission=Ag,
    )
...
def test_tmm_rt_methods():
    ...
    rtstr = rt_structure(
        [front_surf, back_surf], [Ge], [bulkthick], Air, Ag, options, use_TMM=True
    )
    # RT + TMM
    ...
    assert int_front_rt == approx(
        np.sum(result_RT_only["A_per_interface"][0], axis=1), rel=0.02
    )
    assert int_front_arm == approx(np.sum(results_per_layer_front_RT, axis=1), rel=0.02)
    assert int_front_rt == approx(int_front_arm, abs=0.05)

    assert result_RT_only["R"] == approx(results_RT[0]["R"][0], abs=0.05)
    assert result_RT_only["T"] == approx(results_RT[0]["T"][0], abs=0.05)
    assert result_RT_only["A_per_interface"][0] == approx(
        results_per_layer_front_RT, abs=0.05
    )
    assert result_RT_only["A_per_layer"][:, 0] == approx(
        results_RT[0]["A_bulk"][0], abs=0.05
    )
